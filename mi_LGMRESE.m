% Modificado por geem (agosto 2018)
function [vec_sol, vecnormy, xx]=mi_LGMRESE(A,b,mLE, dLE, lLE, max_ciclos, tol, alpha, delta, color, print, Name_Matrix)        %Time Control 
n = size(A,1);
opts.tol= eps;
maxit=max_ciclos;
x0=zeros(n,1);
...
%         j=[];
        ...
        lognormy(size(logres,1)+1,:)=norm_y;
            ...
            if norm(b-A*xm)/res(1,1) <tol  || size(logres,1)==maxit   %empleando ï¿½ltima componente de g como residuo
